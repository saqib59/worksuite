<div class="col-sm-12">
    <x-form id="process-employee-data-form">
        <div class="add-client bg-white rounded">
            ...
            <x-form-actions>
                <x-forms.button-primary id="process-employee-form" disabled class="mr-3" icon="check">@lang('app.submit')
    ...
    <div class="w-100 border-top-grey justify-content-start px-4 py-3 bg-white rounded d-none" id="afterProcessing">
        <x-forms.link-primary :link="route('employees.index')">@lang('app.backToEmployees')</x-forms.link-primary>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('.select-picker').selectpicker();
        ...
        function getProgress(batchId){
            ...
            $('#process-employee-data-form').hide();
            $('#afterSubmitting').show();
            var url = "{{ route('import.process.progress', ['import_employee', ':batchId']) }}";
        ...
        function getQueueException() {
            var url = "{{ route('import.process.exception', 'import_employee') }}";
    ...
    $(document).ready(function() {
        ...
        $('body').on('click', '#process-employee-form', function() {
            const url = "{{ route('employees.import.process') }}";

            $.easyAjax({
                url: url,
                container: '#process-employee-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                buttonSelector: "#process-employee-form",
                data: $('#process-employee-data-form').serialize(),
                success: function (response) {
