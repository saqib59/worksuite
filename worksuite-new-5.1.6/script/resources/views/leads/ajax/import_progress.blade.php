<div class="col-sm-12">
    <x-form id="process-lead-data-form">
        <div class="add-lead bg-white rounded">
            ...
            <x-form-actions>
                <x-forms.button-primary id="process-lead-form" disabled class="mr-3" icon="check">@lang('app.submit')
    ...
    <div class="w-100 border-top-grey justify-content-start px-4 py-3 bg-white rounded d-none" id="afterProcessing">
        <x-forms.link-primary :link="route('leads.index')">@lang('app.backToLead')</x-forms.link-primary>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('.select-picker').selectpicker();
        ...
        function requiredMatchAction() {
            ...
            if (requiredMatched.length == 0) {
                ...
                $("#process-lead-form").removeAttr("disabled");
            } else {
        ...
        function getProgress(batchId){
            ...
            $('#process-lead-data-form').hide();
            $('#afterSubmitting').show();
            var url = "{{ route('import.process.progress', ['import_lead', ':batchId']) }}";
        ...
        function getQueueException() {
            var url = "{{ route('import.process.exception', 'import_lead') }}";
    ...
    $(document).ready(function() {
        ...
        if (getUnMatched() == 0) {
            ...
            $("#process-lead-form").attr("disabled", "disabled");
            showColumnBox(columnID);
        }

        requiredMatchAction();

        $('body').on('click', '#process-lead-form', function() {
            const url = "{{ route('leads.import.process') }}";

            $.easyAjax({
                url: url,
                container: '#process-lead-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                buttonSelector: "#process-lead-form",
                data: $('#process-lead-data-form').serialize(),
                success: function (response) {
