...
<div class="row py-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        ...
        <div class="d-flex" id="table-actions">
            @if (($addTaskPermission == 'all' || $addTaskPermission == 'added' || $project->project_admin == user()->id) && !$project->trashed())
                    ...
                    <div class="select-box py-2 px-0 mr-3">
                        ...
                        <select class="form-control select-picker" name="status" id="status">
                            ...
                            @foreach ($taskBoardStatus as $status)
                                <option value="{{ $status->id }}">{{ $status->slug == 'completed' || $status->slug == 'incomplete' ? __('app.' . $status->slug) : ucwords($status->column_name) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <!-- STATUS END -->
