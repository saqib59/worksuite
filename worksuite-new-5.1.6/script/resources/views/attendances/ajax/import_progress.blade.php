<div class="col-sm-12">
    <x-form id="process-attendance-data-form">
        <div class="add-attendance bg-white rounded">
            ...
            <x-form-actions>
                <x-forms.button-primary id="process-attendance-form" disabled class="mr-3" icon="check">@lang('app.submit')
    ...
    <div class="w-100 border-top-grey justify-content-start px-4 py-3 bg-white rounded d-none" id="afterProcessing">
        <x-forms.link-primary :link="route('attendances.index')">@lang('app.backToAttendance')</x-forms.link-primary>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $('.select-picker').selectpicker();
        ...
        function requiredMatchAction() {
            ...
            if (requiredMatched.length == 0) {
                ...
                $("#process-attendance-form").removeAttr("disabled");
            } else {
        ...
        function getProgress(batchId){
            ...
            $('#process-attendance-data-form').hide();
            $('#afterSubmitting').show();
            var url = "{{ route('import.process.progress', ['import_attendance', ':batchId']) }}";
        ...
        function getQueueException() {
            var url = "{{ route('import.process.exception', 'import_attendance') }}";
    ...
    $(document).ready(function() {
        ...
        if (getUnMatched() == 0) {
            ...
            $("#process-attendance-form").attr("disabled", "disabled");
            showColumnBox(columnID);
        }

        requiredMatchAction();

        $('body').on('click', '#process-attendance-form', function() {
            const url = "{{ route('attendances.import.process') }}";

            $.easyAjax({
                url: url,
                container: '#process-attendance-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                buttonSelector: "#process-attendance-form",
                data: $('#process-attendance-data-form').serialize(),
                success: function (response) {
