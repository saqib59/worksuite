    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('check', function ($row) {
                $disabled = ($row->gateway == null || $row->gateway == 'Offline' || $row->status == 'failed') ? '' : 'disabled';

                return '<input type="checkbox" '. $disabled .' class="select-table-row disabled"  id="datatable-row-' . $row->id . '"  name="datatable_ids[]" value="' . $row->id . '" onclick="dataTableRowCheck(' . $row->id . ')">';
            })
