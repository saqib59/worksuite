    ...
    public function handle()
    {
        ...
        if($setting && $setting->status == 'active') {
            ...
            if(count($backups) == 0){
                ...
                if($dateDifference >= $backupSetting->backup_after_days && Carbon::now()->setTimezone(global_setting()->timezone)->format('H:i:s') >= \Carbon\Carbon::createFromFormat('H:i:s', $backupSetting->hour_of_day)->format('H:i:s')){
                    Artisan::call('backup:run --only-db');
                }
            }
